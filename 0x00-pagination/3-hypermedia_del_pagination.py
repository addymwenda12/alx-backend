...
class Server:
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        """Retrieves a page of the dataset with the given index and page size.

        Args:
            index (int, optional): The starting index for the page.
            Defaults to None.
            page_size (int, optional): The number of items to
            include in the page.Defaults to 10.

        Returns:
            dict: A dictionary containing the page data,
            index, next index, and page size.
        """
        indexed_dataset = self.indexed_dataset()
        assert index is None or (index >= 0 and index <
                                 len(indexed_dataset)), "Index out of range"
